...
#define DECLARE_ACMODULE(ModuleName, ModuleNameString, EXPORT_TAG) \
	extern const std::string ModuleNameString; \
	EXPORT_TAG I##ModuleName##Module* GetOrCreate##ModuleName##Module(); \
	EXPORT_TAG void Destroy##ModuleName##Module();


#define IMPLEMENT_ACMODULE(ModuleName, ModuleNameString) \
	const std::string ModuleNameString = std::string(#ModuleName) + std::string("Module"); \
	...
	if (NULL != g_p##ModuleName##Module) \
		...
		bool bInitOk = g_p##ModuleName##Module->Initialise(ModuleNameString); \
		if (!bInitOk) \
		{ \
